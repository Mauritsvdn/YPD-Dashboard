 ...
 * - Maandsalaris-range (bv. "3000 tot 3500 per maand") → "€3.000,- tot €3.500,- bruto/maand".
...
export function formatSalaris(salaris: string | null | undefined): string {
  ...
  const rangeMatch = trimmed.match(
    /^€?\s*(\d{1,3}(?:[.\s]\d{3})*|\d+)\s*(?:-|–|tot|t\/m)\s*€?\s*(\d{1,3}(?:[.\s]\d{3})*|\d+)(?:\s*(?:bruto)?\s*(?:per|\/)\s*maand)?$/i
  );
  if (rangeMatch) {
    const van = parseInt(rangeMatch[1].replace(/[.\s]/g, ""), 10);
    const tot = parseInt(rangeMatch[2].replace(/[.\s]/g, ""), 10);
    if (van < 12000 && tot < 12000) {
      return `€${van.toLocaleString("nl-NL")},- tot €${tot.toLocaleString("nl-NL")},- bruto/maand`;
    }
  }
  if (/^\d+$/.test(trimmed)) {
...
// ── Herbruikbare stijlen als CSS-classes (1× in de <style> gedefinieerd → geen herhaalde inline
// styles per kandidaat, wat de mailgrootte fors verkleint zonder het uiterlijk te wijzigen).
// De desktop-waarden staan in de basisregels in de <style>; de mobiele media-query en dark-mode
// regels overschrijven ze net als voorheen met !important.

/** Render een bullet-lijstje als tabelrijen. */
const bullets = (items: string[]): string =>
  items.map((i) => `<tr><td class="ypd-bullet ypd-bul">• ${i}</td></tr>`).join("");

/** Render een paars uppercase sectielabel. */
const label = (t: string): string =>
  `<p class="ypd-text-purple ypd-label">${t}</p>`;

function categorieAnchor(cat: string, index: number): string {
  const slug = cat.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
  return `cat-${index + 1}-${slug || "categorie"}`;
}
...
export function generateMailchimpHtml(
  ...
  const navLinks = categorieenInMailing
    .map((cat, index) => {
      const anchor = categorieAnchor(cat, index);
    ...
    .map((cat, index) => {
      ...
      const kandidaatCards = kands
        .map((k) => {
          ...
          const linkerKolom =
            `<div class="ypd-col" align="left" style="max-width:340px;">` +
          ...
          const rechterKolom =
            `<div class="ypd-col" align="left" style="max-width:250px;">` +
              `<table width="100%" align="left" cellpadding="0" cellspacing="0" border="0"><tr>` +
              `<td class="ypd-col-r">` +
          ...
          const bijzonderheden = k.bijzonderheden
            ? `<tr><td class="ypd-bg-tint-2 ypd-bijz" bgcolor="#faf8fc">` +
          ...
          return (
            `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" class="ypd-card" bgcolor="#ffffff">` +
              // Kandidaat header
              `<tr><td class="ypd-chead">` +
                `<p class="ypd-text-dark ypd-name">${formatNaam(k.neepnaam, k.leeftijd)}</p>` +
                ...
                `<p class="ypd-text-muted ypd-meta">` +
              ...
              bijzonderheden +
              // CTA
              `<tr><td align="right" class="ypd-cfoot">` +
                `<a href="${cvUrl}" class="ypd-cta">Ik wil een CV ontvangen →</a>` +
              `</td></tr>` +
            `</table>`
          );
        })
      ...
      return (
        `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:28px 0 12px 0;"><tr>` +
          `<td style="padding:10px 18px;border-radius:8px;background:linear-gradient(90deg,#f1eafa,#fce8f0);">` +
            `<a id="${anchor}" name="${anchor}" style="display:block;height:1px;line-height:1px;font-size:1px;text-decoration:none;color:transparent;">&nbsp;</a>` +
